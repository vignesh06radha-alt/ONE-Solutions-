...
import { LayoutDashboard, Flag, Handshake, Building, Map, Bot, BarChart, Settings } from 'lucide-react';
...
const TopNav: React.FC<TopNavProps> = ({ currentView, setView }) => {
    ...
    return (
        <nav className="bg-[#111111] border-b border-gray-800 px-2 sm:px-4">
            <div className="flex items-center space-x-1 overflow-x-auto whitespace-nowrap scrollbar-hide">
                {filteredNavItems.map(item => (
                    <a
                        ...
                        className={`flex items-center px-3 py-3 text-sm font-medium transition-all duration-200 group border-b-2 rounded-t-sm ${
                            currentView === item.path 
                            ? 'border-green-400 text-green-400 bg-gray-900/50' 
                            : 'border-transparent text-gray-400 hover:text-white hover:bg-gray-800/60'
                        }`}
                    >
                        <item.icon className={`w-5 h-5 mr-2 transition-colors duration-200 ${currentView === item.path ? 'text-green-400' : 'text-gray-500 group-hover:text-gray-300'}`} />
                        <span>{item.name}</span>
                    </a>
                ))}
            </div>
